                    ...
                    <?php	//in deze php wordt er in de loginpagina gekeken of de gebruiker al is ingelogd door de session u_id te checken
                            if (isset($_SESSION['u_id'])) {                                     //form action geeft de url aan waar de verzonden info heen gaat
                                    echo '<form action="includes/logout.inc.php" method="POST">      
                                          <button type="submit" name="submit">Uitloggen</button></form>'; //submit zorgt ervoor dat de gebruiker naar de logout.inc.php gaat en daar worden de sessions leeg gemaakt en de gebruiker gaat terug naar de indez
                            }else{
                                    echo '<form action="includes/login.inc.php" method="POST">      
                                          <input type="text" name="uid" placeholder="Gebruikersnaam/e-mail">
                                          <input type="text" name="pwd" placeholder="Wachtwoord">
                                          <button type="submit" name="submit">Inloggen</button></form>
